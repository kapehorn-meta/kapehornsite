...
export class GradientBackground {
  ...
  init() {
    this.canvas = document.querySelector('[data-gradient-canvas]');
  ...
  drawGradientLayers(width, height) {
    ...
    layers.forEach((layer, index) => {
      ...
      // Interpolate colors
      const color1 = this.interpolateColor(currentColor, nextColor, colorProgress);
  ...
  addGradientStops(gradient, color1, color2, opacity) {
    gradient.addColorStop(0, this.createRGBA(color1, opacity));
    gradient.addColorStop(0.15, this.createRGBA({
      r: color1.r * 0.8 + color2.r * 0.2,
      g: color1.g * 0.8 + color2.g * 0.2,
      b: color1.b * 0.8 + color2.b * 0.2
    }, opacity * 0.9));
    gradient.addColorStop(0.3, this.createRGBA(color2, opacity * 0.7));
    gradient.addColorStop(0.45, this.createRGBA({
      r: color2.r * 0.8 + color1.r * 0.2,
      g: color2.g * 0.8 + color1.g * 0.2,
      b: color2.b * 0.8 + color1.b * 0.2
    }, opacity * 0.6));
    gradient.addColorStop(0.6, this.createRGBA(color1, opacity * 0.5));
    gradient.addColorStop(0.75, this.createRGBA({
      r: color1.r * 0.6 + color2.r * 0.4,
      g: color1.g * 0.6 + color2.g * 0.4,
      b: color1.b * 0.6 + color2.b * 0.4
    }, opacity * 0.3));
    gradient.addColorStop(0.9, this.createRGBA({
      r: color2.r * 0.3 + color1.r * 0.7,
      g: color2.g * 0.3 + color1.g * 0.7,
      b: color2.b * 0.3 + color1.b * 0.7
    }, opacity * 0.1));
    gradient.addColorStop(1, this.createRGBA(color2, 0));
  }
